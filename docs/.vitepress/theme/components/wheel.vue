      ...
      <div class="wheel-wrapper">
        ...
        >
          <div
            ...
            :key="index"
            :class="['slice', { 'is-active': activeIndex === index }]"
        ...
        <div class="drawer-body">
          ...
          <div class="list-container">
            <div v-for="(item, index) in currentFoods" :key="index" class="list-item">
              ...
              <input v-model.number="item.weight" class="edit-weight" />
...
const activeIndex = ref(-1) // 用于移动端点击选中状态
...
function getSliceStyle(s) {
  return { 
    transform: `translate(-50%, -50%) rotate(${s.midAngle}deg) translateY(-35%)` 
  }
}
...
.tab-item { border: none; padding: 10px 20px; border-radius: 10px; cursor: pointer; background: transparent; color: #666; font-weight: bold; position: relative; transition: 0.2s; font-size: 14px;}
.tab-item.active { background: #fff; color: #ff4757; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
.time-tag { position: absolute; top: -5px; right: -2px; background: #2ed573; color: #fff; font-size: 8px; padding: 1px 4px; border-radius: 4px; }

.main-content { display: flex; flex-direction: column; align-items: center; width: 100%; margin-top: -10px;}
.title { color: #2f3542; margin-bottom: 25px; font-size: 1.4rem; }

/* 转盘容器：使用 vw 保证移动端居中且不溢出 */
.wheel-wrapper { 
  position: relative; 
  width: 420px; 
  height: 420px; 
...
.pointer { position: absolute; top: -15px; left: 50%; transform: translateX(-50%); width: 0; height: 0; border-left: 15px solid transparent; border-right: 15px solid transparent; border-top: 28px solid #ff4757; z-index: 10; }

.wheel { width: 100%; height: 100%; border-radius: 50%; border: 6px solid #fff; box-shadow: 0 10px 40px rgba(0,0,0,0.1); position: relative; overflow: hidden; }
...
.text { 
  ...
  text-align: center;
  /* 解决上下居中的核心：让文字块自身垂直居中在轨道线上 */
  transform: translateY(50%); 
}

.spin-btn { background: #ff4757; color: #fff; border: none; padding: 14px 70px; font-size: 1.2rem; border-radius: 50px; cursor: pointer; font-weight: bold; box-shadow: 0 8px 20px rgba(255,71,87,0.3); transition: 0.2s; }
.spin-btn:active { transform: scale(0.95); }
.spin-btn:disabled { background: #ccc; cursor: not-allowed; }

.result-box { height: 60px; margin-top: 20px; text-align: center; }
.result { font-size: 20px; font-weight: bold; color: #2f3542; }
...
.config-toggle {
  ...
  box-shadow: 0 4px 15px rgba(0,0,0,0.1); cursor: pointer; z-index: 99; 
}
...
.drawer-content { position: absolute; right: -320px; top: 0; bottom: 0; width: 300px; background: #fff; padding: 25px; transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: flex; flex-direction: column; }
...
.drawer-header button { border: none; background: none; font-size: 20px; cursor: pointer; }
...
.add-form button { background: #2f3542; color: #fff; border: none; padding: 0 12px; border-radius: 6px; cursor: pointer; }
...
.edit-name { border: none; background: transparent; flex: 1; min-width: 0; font-size: 14px; }
.edit-weight { width: 35px; border: 1px solid #eee; text-align: center; font-size: 12px; }
